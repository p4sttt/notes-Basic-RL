...
$$
\nabla_{\theta} \ln P_{\theta}(\tau)=\nabla_{\theta}
\left(\ln p(s_{0})+\sum_{t=0}^{T}\ln \pi_{\theta}(a_{t} \mid s_{t})+\sum_{t=0}^{T}\ln P(s_{t+1} \mid s_{t}, a_{t})\right)
$$
...
\left[\sum_{t=0}^{T}\nabla_{\theta} \ln \pi_{\theta}(a_{t} \mid s_{t})R(\tau)\right]
$$
...
$$
Q^\ast(s, a)=\mathbb{E}\left[r_t+\gamma\max_{a'} Q^\ast(s_{t+1}, a')\mid s_t = s, a_t = a\right]
$$
...
$$
\nabla_{\theta} J(\pi_{\theta}) \approx \mathbb{E} \left[ \nabla_{\theta} \ln \pi_{\theta}(a_t \mid s_t) \delta_t \right]
$$
